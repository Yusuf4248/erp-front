import { useStudentAttendanceByGroupId } from "@hooks";
import type { LessonType } from "@types";
import { Tooltip } from "antd";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { useRef, useState } from "react";
import StudentInfo from "../../components/students/student-info-modal";
import StudentAttendaceModal from "../../components/students/student-modal"
const windowWidth = window.innerWidth;
const StudentAttendance = ({ studentId, groupId }: any) => {
	const [open, setOpen] = useState(false);
	const [update, setUpdate] = useState<LessonType | null>(null);
	const { data, isLoading } = useStudentAttendanceByGroupId(studentId, groupId);
	const attendance = data?.data?.attendances || [];
	...
	return (
		<div
			hidden={isLoading}
			className="w-full bg-white rounded-2xl  border border-gray-100 overflow-hidden mb-5 "
		>
			...
			{open && <StudentAttendaceModal open={open} toggle={toggle} update={update} />}

...
const SingleGroup = () => {
	...
	const { dataById } = useGroupById(Number(id));
	const { students } = useGroupStudents(Number(id));
	const { lessons } = useGroupLessons(Number(id));
	const { teachers } = useGroupTeachers(Number(id));
